...
export async function parseToolData(url){
      ...
      const scrappedData = await scrapeURL(normalizedUrl);
      if(!scrappedData){
        throw new Error('Error parsing URL');
        return ;
      }
      const bestMatch = scrappedData[0];
      const refinedToolData = await refineToolData(normalizedUrl,bestMatch);
      if(!refinedToolData){
         throw new Error('Error parsing URL');
        return ;
      }
...
async function scrapeURL(url) {
  ...
  const serpPromise = new Promise((resolve, reject) => {
    getJson(
      {
        engine: "google",
  ...
  const timeout = new Promise((_, reject) =>
    setTimeout(() => reject(new Error("SerpAPI request timed out")), 20000)
  );

  return Promise.race([serpPromise, timeout]);
}
